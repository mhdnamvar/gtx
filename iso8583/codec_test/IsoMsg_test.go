...
func TestIsoMsgGet(t *testing.T) {
	...
	assert.Equal(t, "", s)
	assert.Equal(t, codec.FieldNotFound, err)
...
func TestIsoMsgEncodeAscii(t *testing.T) {
	isoMsg := sampleIsoMsg()
	bytes, err := isoMsg.Encode(codec.Ascii87)
...
func TestIsoMsgParse(t *testing.T) {
	...
	err := isoMsg.Parse(codec.Ascii87, s)
	assert.Equal(t, err, nil)
...
func sampleIsoMsg() *codec.IsoMsg {
	isoMsg := codec.NewIsoMsg()
	isoMsg.Set(0, "210")
...
func TestIsoMsgDump(t *testing.T) {
	s, err := sampleIsoMsg().Dump(codec.Ascii87)
	assert.Equal(t, nil, err)
	fmt.Println(s)
}
...
func TestIsoMsgEncodeBinary(t *testing.T) {
	...
	isoMsg.Set(128, "2D2A98F12D2A98F1")
	bytes, err := isoMsg.Encode(codec.Binary87)
	assert.Equal(t, err, nil)
	log.Printf("%X", bytes)
}
