...
type LLLBStringB struct {
	Len  *NumericB
	Data *StringB
}

func NewLLLBStringB(id string, label string, padding IsoPadding, paddingStr string, size int) *LLLBStringB {
	if size > LLLVarB.MaxValue {
		panic(iso8583.InvalidLengthError)
	}
	return &LLLBStringB{
		DefaultNumericB(LLLVarB.Size),
		&StringB{
			Id:          id,
			Label:       label,
			Encoding:    EncodingA,
...
func DefaultLLLBStringB(size int) *LLLBStringB {
	if size > LLLVarB.MaxValue {
		panic(iso8583.InvalidLengthError)
	}
...
func (codec *LLLBStringB) Encode(s string) ([]byte, error) {
	err := codec.Check(s)
	if err != nil {
		fmt.Println("==> 5")
		return nil, err
	...
	l, err := codec.Len.Encode(fmt.Sprintf("%04d", len(b)))
	if err != nil {
		fmt.Println("==> 7")
		return nil, err
	}
...
func (codec *LLLBStringB) Decode(b []byte) (string, int, error) {
...
func (codec *LLLBStringB) Check(s string) error {
...
func (codec *LLLBStringB) LenSize() int {
	return LLLVarB.Size
}
