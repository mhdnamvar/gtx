...
type LLLANumericA struct {
	Len  *NumericA
	Data *NumericA
}

func NewLLLANumericA(id string, label string, padding IsoPadding, paddingStr string, size int) *LLLANumericA {
...
func DefaultLLLANumericA(size int) *LLLANumericA {
	if size > LLLVarA.MaxValue {
		panic(iso8583.InvalidLengthError)
	}
...
func (codec *LLLANumericA) Encode(s string) ([]byte, error) {
	err := codec.Check(s)
	if err != nil {
		return nil, err
	}
...
func (codec *LLLANumericA) Decode(b []byte) (string, int, error) {
...
func (codec *LLLANumericA) Check(s string) error {
...
func (codec *LLLANumericA) LenSize() int {
	return LLLVarA.Size
}
