...
func DefaultLLLBNumericB(size int) *LLLBNumericB {
	if size > LLLVarB.MaxValue {
		panic(iso8583.InvalidLengthError)
	}
...
func (codec *LLLBNumericB) Encode(s string) ([]byte, error) {
	err := codec.Check(s)
	if err != nil {
		fmt.Println("==> 5")
		return nil, err
...
func (codec *LLLBNumericB) Decode(b []byte) (string, int, error) {
...
func (codec *LLLBNumericB) Check(s string) error {
