...
type LLBStringB struct {
	Len  *NumericB
	Data *StringB
}

func NewLLBStringB(id string, label string, padding IsoPadding, paddingStr string, size int) *LLBStringB {
...
func DefaultLLBStringB(size int) *LLBStringB {
	if size > LLVarB.MaxValue {
		panic(iso8583.InvalidLengthError)
	}
...
func (codec *LLBStringB) Encode(s string) ([]byte, error) {
	err := codec.Check(s)
	if err != nil {
		fmt.Println("==> 5")
		return nil, err
...
func (codec *LLBStringB) Decode(b []byte) (string, int, error) {
...
func (codec *LLBStringB) Check(s string) error {
...
func (codec *LLBStringB) LenSize() int {
	return LLVarB.Size
}
