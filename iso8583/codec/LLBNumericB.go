...
type LLBNumericB struct {
	Len  *NumericB
	Data *NumericB
}

func NewLLBNumericB(id string, label string, padding IsoPadding, paddingStr string, size int) *LLBNumericB {
	if size > LLVarB.MaxValue {
		panic(iso8583.InvalidLengthError)
	}
	return &LLBNumericB{
		DefaultNumericB(LLVarB.Size),
		&NumericB{
			Id:          id,
			Label:       label,
			Encoding:    EncodingA,
...
func DefaultLLBNumericB(size int) *LLBNumericB {
	if size > LLVarB.MaxValue {
		panic(iso8583.InvalidLengthError)
	}
	data := DefaultNumericB(size)
	data.MinLen = 0
	return &LLBNumericB{
		DefaultNumericB(LLVarB.Size),
		data,
...
func (codec *LLBNumericB) Encode(s string) ([]byte, error) {
	err := codec.Check(s)
	if err != nil {
		fmt.Println("==> 5")
		return nil, err
...
func (codec *LLBNumericB) Decode(b []byte) (string, int, error) {
...
func (codec *LLBNumericB) Check(s string) error {
...
func (codec *LLBNumericB) LenSize() int {
	return LLVarB.Size
}
